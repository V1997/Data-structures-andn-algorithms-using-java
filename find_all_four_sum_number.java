  ...
  public int compare(Result result1, Result result2) {
    if (result1.p1.first == result2.p1.first) {
      if (result1.p1.second == result2.p1.second) {
        ...
        } else return result1.p2.first - result2.p2.first;
      } else return result1.p1.second - result2.p1.second;
    } else return result1.p1.first - result2.p1.first;
  }
}
    ...
    for (Pair p1 : value1) {
      for (Pair p2 : value2) {
        if (
          ...
          Arrays.sort(temp);
          resultSet.add(
            new Result(new Pair(temp[0], temp[1]), new Pair(temp[2], temp[3]))
          );
        }
      }
    }
